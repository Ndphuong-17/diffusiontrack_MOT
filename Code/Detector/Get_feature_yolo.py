...
def get_object_features(feat_list, idxs):
    ...
    s = [x.shape[1] for x in feat_list]
    s = min(s) # smallest vector length (64 for YOLOv8)
    obj_feats = torch.cat([x.permute(0, 2, 3, 1).reshape(-1, s, x.shape[1] // s).mean(dim=-1) for x in feat_list], dim=0)
